...
`demo` / `password123`). *Free Render instance, so if it's been idle the first load can
take ~50s to wake up.*
...
It's built as a **backend-focused resume project**: a layered TypeScript API, a
...
and ships as a single production image that's been deployed to both Render and Azure.
...
- **Cloud & infra.** Fully Dockerized local dev, a single multi-stage production image,
  and scripted deploys to Render (always-on) and Azure (one-time proof).
...
| Dev/Infra | Docker Compose · Render · Azure                       |

## Quick start

Prerequisite: **[Docker Desktop](https://www.docker.com/products/docker-desktop/)** running.
Node, Postgres, and Python all run inside containers, so there's nothing else to install.
...
**Authentication.** Register or log in with username, email, and password. Passwords are
...
**Load Data.** Upload the Households, Products, and Transactions CSVs from the browser; the
...
- **Offline ML to the database.** Python jobs read transactions, train models, and write
  results (`basket_rules`, `household_churn`, and so on) into Prisma-owned tables. The API
  only ever serves precomputed rows, so the runtime stays a pure TypeScript service.
...
The ML container sits behind a Compose **profile**, so it never starts with
...
| # | Requirement                                             | Where                                              |
|---|---------------------------------------------------------|----------------------------------------------------|
| 1 | Web server + interactive page (username/password/email) | Auth module + Render/Azure deploy                  |
...
| + | Cloud deployment                                        | Live on Render; one-time Azure proof (below)       |

## Engineering decisions & trade-offs

Recorded as ADRs under **[docs/adr/](docs/adr/)**. A few highlights:

- **Express over NestJS**, and **polyglot** (TypeScript API + offline Python ML) over
  Python-only, for transparency while learning and because TS is the stronger backend
  signal.
- **Layered architecture with HTTP-seam integration tests.** Tests assert external
  behavior over real HTTP against a dedicated test database.
- **JWT over httpOnly session cookies**, **bcrypt over argon2id**: interview-standard,
  dependency-light choices, with the trade-offs written up in the ADRs.
- **Offline ML precomputed into the database**, which keeps the runtime a pure TS API and
...
- **Data loading is destructive.** An upload (or `npm run load`) truncates and reloads; it
  replaces the dataset rather than appending. That's fine for a single-user analytics app,
  but not multi-tenant-safe.
- **Churn is recency-driven.** The label ("no purchase in the final 90-day window") is
  mechanically related to recency at the cutoff, so the model's high ROC-AUC reflects an
  easy, recency-dominated signal. It's a sound, standard framing, not a claim of deep
  predictive power. See [ADR-0006](docs/adr/0006-churn-cutoff-window-label.md).
...
boot) with two documented deployments:

- **Live demo on Render + Neon (always-on, free).** The public link at the top runs the
...
rather than an always-on service; the durable deliverable is this repo and the screenshots.
A single production image runs on **Azure App Service for Containers** against **Azure
Database for PostgreSQL**, all in one resource group. `infra/azure/deploy.sh` provisions it
and `infra/azure/teardown.sh` removes everything. Full walkthrough:
...
├── docs/                    # ADRs, deploy guides + screenshots, agent config
...
- Source folders are bind-mounted into the containers, so code edits hot-reload. After
  adding a **new** API route or file, restart the API container
  (`docker compose restart api`), since file-watch across the macOS bind mount can miss new
  files.
- In dev, the Vite server proxies `/api/*` to the API container (same-origin, no CORS); in
  prod, Express serves the React build at the same origin, so the frontend code is
  identical in both.
